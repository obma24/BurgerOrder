...
def new_order():
    ...
    orders.append(full_order)  # Add the new order to our list
...
def get_orders():
    ...
    """
    return jsonify(orders)  # Send back the orders as JSON
...
def cancel_order(index):
    ...
    if 0 <= index < len(orders):
        orders.pop(index)  # Remove the order from the list
